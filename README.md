...
- 添加了游戏内输入法.
- 内置可扩展的自定义 UI 框架，支持图形化设置、桌面小组件等.

![图形化设置](<img/UI/图形化设置.png>)

![示例窗口管理和自定义相机dom](<img/UI/示例窗口管理和自定义相机dom.png>)
...
### 🖥️ 自定义 UI 框架
- **⚙️ 图形化设置**：无需手动编辑配置文件，直接在游戏内可视化修改所有设置
- **🧩 桌面小组件**：支持天气、相机控制器等可拖拽窗口小组件
- **🔤 输入法设置面板**：可视化调整 IME 模糊效果、候选词数量等参数
- **🔍 场景浏览器**：实时查看和调试游戏内 UI 元素树
- **🔌 插件式扩展**：窗口管理器支持开发者编写自定义插件
...
#### Bilibili 音乐模块 — 作者: [@xiaogouqianqian](https://github.com/xiaogouqianqian)
- **🔓 二维码登录**：内置 Bilibili 二维码扫码登录
- **📂 收藏夹同步**：自动加载所有 B 站收藏夹，每个收藏夹生成独立标签
- **🎧 流式播放**：边下边播，10 秒环形缓冲，下载完成后支持 seek
- **🖼️ 智能封面**：自动加载视频封面并居中裁切为正方形
- **⚙️ 可配置翻页延迟**：防止 412 限流，默认 300ms
- **API**: 使用 Bilibili 官方 API（`api.bilibili.com`、`passport.bilibili.com`）
...
ChillPatcher 内置了一套基于 [OneJS](https://onejs.com/) 的自定义 UI 系统，允许用户通过可视化界面管理设置、查看模块信息，以及添加桌面小组件。
...
UI 系统基于 [OneJS](https://onejs.com/)，在 Unity UIToolkit 上运行 JavaScript/TypeScript。

#### 架构概览

- **渲染层**：OneJS 将 Preact 虚拟 DOM 映射到 Unity UIToolkit 的 VisualElement 树
- **框架**：使用 [Preact](https://preactjs.com/)（轻量 React 替代）作为 UI 框架，配合自定义 DOM 适配层
- **游戏 API**：通过 `chill` 全局对象访问游戏功能（配置读写、音频控制、事件订阅等）
- **C# 互操作**：通过 `CS` 全局对象直接调用任意 C# 类和方法

#### 开发环境

每个 UI 实例是 `ui/` 目录下的独立 Node.js 项目（含 `package.json`）。游戏启动时自动执行：

1. **`npm install`** — 安装依赖（仅在 `node_modules` 不存在时）
2. **`npm run build`** — 通过 esbuild 编译 TypeScript/JSX 为单文件 bundle（仅在输出不存在时）
3. **esbuild watch** — 启动文件监听，源码修改后自动重新编译

> 需要系统安装 Node.js。如果未安装，将使用预编译的 bundle，不影响运行。

#### 热重载

UI 实例运行时每 0.3 秒检查入口文件（`@outputs/esbuild/app.js`）的修改时间。当 esbuild watch 重新编译后，JS 引擎自动重新加载，无需重启游戏。

#### 示例项目

`ui/` 目录下包含两个完整的参考项目：

- **`ui/default/`** — 主 UI 实例，包含图形化设置面板、模块管理器、IME 设置、场景浏览器等
- **`ui/window-manager/`** — 窗口管理器实例，提供可拖拽窗口系统和插件机制，内含天气小组件和相机控制器示例,如果你想要做小组件,可以遵循这个项目中的插件实现,后续将得到无成本的兼容

可以参考这两个项目来了解 UI 组件的开发方式和游戏 API 的用法。
...
- **fdk-aac** ([Fraunhofer FDK AAC](https://github.com/mstorsjo/fdk-aac)) - Fraunhofer FDK AAC License
- **minimp4** ([minimp4](https://github.com/lieff/minimp4)) - CC0 1.0 Universal (Public Domain)
...
### 模块贡献者
- [@xiaogouqianqian](https://github.com/xiaogouqianqian) - Bilibili 音乐模块原作者
- [@JinCao-0721](https://github.com/JinCao-0721) - QQ 音乐模块原作者
...
- [dr_libs](https://github.com/mackron/dr_libs) - FLAC/WAV/MP3 音频解码
- [fdk-aac](https://github.com/mstorsjo/fdk-aac) - AAC 音频解码支持
- [minimp4](https://github.com/lieff/minimp4) - MP4/FMP4 解封装支持
